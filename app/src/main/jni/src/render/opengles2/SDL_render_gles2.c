...
GL_CheckAllErrors(const char *prefix, SDL_Renderer *renderer, const char *file, int line,
                  const char *function) {
    GLES2_DriverContext *data = (GLES2_DriverContext *) renderer->driverdata;
    int ret = 0;
    ...
    for (;;) {
        ...
        if (error != GL_NO_ERROR) {
            ...
            SDL_SetError("%s: %s (%d): %s %s (0x%X)", prefix, file, line, function,
                         GL_TranslateError(error), error);
            ret = -1;
        } else {
            break;
        }
...
GLES2_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture) {
    ...
    renderdata->glTexImage2D(data->texture_type, 0, format, texture->w, texture->h, 0, format, type,
                             NULL);
...
GLES2_SetRenderTarget(SDL_Renderer *renderer, SDL_Texture *texture) {
    ...
    if (texture == NULL) {
        ...
        data->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                     texturedata->texture_type, texturedata->texture, 0);
        /* Check FBO status */
...
GLES2_CacheProgram(SDL_Renderer *renderer, GLES2_ShaderCacheEntry *vertex,
    ...
    data->glUniformMatrix4fv(entry->uniform_locations[GLES2_UNIFORM_PROJECTION], 1, GL_FALSE,
                             (GLfloat *) entry->projection);
    data->glUniform1i(entry->uniform_locations[GLES2_UNIFORM_TEXTURE],
                      0);  /* always texture unit 0. */
    ...
    if (blendMode != data->current.blendMode) {
        switch (blendMode) {
            ...
            case SDL_BLENDMODE_BLEND:
                ...
                data->glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE,
...
GLES2_SetDrawingState(SDL_Renderer *renderer) {
    ...
    if (!CompareColors(program->color_r, program->color_g, program->color_b, program->color_a, r, g,
                       b, a)) {
        /* Select the color to draw with */
        data->glUniform4f(program->uniform_locations[GLES2_UNIFORM_COLOR], r * inv255f, g * inv255f,
                          b * inv255f, a * inv255f);
        program->color_r = r;
        program->color_g = g;
        program->color_b = b;
...
GLES2_RenderCopy(SDL_Renderer *renderer, SDL_Texture *texture, const SDL_Rect *srcrect,
    ...
    if (!CompareColors(program->modulation_r, program->modulation_g, program->modulation_b,
                       program->modulation_a, r, g, b, a)) {
        data->glUniform4f(program->uniform_locations[GLES2_UNIFORM_MODULATION], r * inv255f,
                          g * inv255f, b * inv255f, a * inv255f);
 ...
 *************************************************************************************************/
static int
GLES2_BindTexture(SDL_Renderer *renderer, SDL_Texture *texture, float *texw, float *texh);
...
//    GLES2_DriverContext *data;
    ...
    renderer = SDL_calloc(1, (SDL_Renderer *) sizeof(struct SDL_Renderer));
    if (!renderer) {
        SDL_OutOfMemory();
        return NULL;
    }
...
//    renderer->info.max_texture_height = value;
    ...
    //LOGE("data->shader_formats[nFormats - 1]:%d\n", data->shader_formats[nFormats - 1]);
#endif /* ZUNE_HD */
