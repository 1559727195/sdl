...
SDL_UpdateFullscreenMode(SDL_Window *window, SDL_bool fullscreen) {
    ...
    if ((display->fullscreen_window == window) == fullscreen) {
        if ((window->last_fullscreen_flags & FULLSCREEN_MASK) ==
...
SDL_SetWindowMinimumSize(SDL_Window *window, int min_w, int min_h) {
    ...
    if (!(window->flags & SDL_WINDOW_FULLSCREEN)) {
        ...
        SDL_SetWindowSize(window, SDL_max(window->w, window->min_w),
                          SDL_max(window->h, window->min_h));
    }
}
...
SDL_SetWindowMaximumSize(SDL_Window *window, int max_w, int max_h) {
    ...
    if (!(window->flags & SDL_WINDOW_FULLSCREEN)) {
        ...
        SDL_SetWindowSize(window, SDL_min(window->w, window->max_w),
                          SDL_min(window->h, window->max_h));
    }
}
...
SDL_CreateWindowFramebuffer(SDL_Window *window) {
    ...
    return SDL_CreateRGBSurfaceFrom(pixels, window->w, window->h, bpp, pitch, Rmask, Gmask, Bmask,
                                    Amask);
}
...
static SDL_bool
SDL_MessageboxValidForDriver(const SDL_MessageBoxData *messageboxdata, SDL_SYSWM_TYPE drivertype) {
    SDL_SysWMinfo info;
